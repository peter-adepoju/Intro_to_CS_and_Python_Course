...
function weekResources(week) {
  ...
  return [
    ["Week overview", `materials/${folder}/README.html`],
    ["Schedule", `materials/${folder}/00_SCHEDULE/week_${String(week).padStart(2, "0")}_schedule.html`],
    ["Textbook reader", `textbook/week-${String(week).padStart(2, "0")}/index.html`],
    ["Notebooks", `${folder}/02_NOTEBOOKS`],
    ["Practice", `${folder}/03_CODING_PRACTICE`],
    ["Assignments", `materials/${folder}/04_ASSIGNMENTS/index.html`],
    ["Resources", `materials/${folder}/07_RESOURCES/week_${String(week).padStart(2, "0")}_cheatsheet.html`],
    ["Tracker", `materials/${folder}/09_PROGRESS_TRACKER/week_${String(week).padStart(2, "0")}_tracker.html`]
  ];
}

const updates = [
  ["June 2026", "Weeks 1-5 are now represented as live course sections on the website."],
  ["June 2026", "Colab notebook links now point to the correct week folders for Weeks 1-5."],
  ["June 2026", "Combined textbook chapters were split into standalone chapter files."],
  ["June 2026", "Week 3 includes the first mini-project materials."],
  ["June 2026", "The site was redesigned as a dark course dashboard with a skills map and richer week cards."],
  ["June 2026", "Textbook chapters are now available as polished web pages with navigation and section links."],
  ["June 2026", "Schedules, assignments, quizzes, resources, setup guides, and trackers now have readable web pages."],
  ["June 2026", "Interactive daily quiz pages now provide scoring, reshuffled options, and answer explanations."]
];

const skillGroups = [
  {
    title: "Foundations",
    note: "The grammar of small Python programs.",
    skills: ["computation", "types", "variables", "expressions", "strings", "input", "f-strings", "branching"]
  },
  {
    title: "Iteration",
    note: "Repeating work clearly and safely.",
    skills: ["while loops", "for loops", "range()", "nested loops", "approximation", "brute force", "break", "continue"]
  },
  {
    title: "Functions",
    note: "Turning repeated ideas into reusable tools.",
    skills: ["def", "parameters", "arguments", "return", "scope", "docstrings", "decomposition", "specifications"]
  },
  {
    title: "Recursion",
    note: "Solving smaller versions of a problem.",
    skills: ["base cases", "termination", "call stack", "recursive cases", "mutual recursion", "iteration comparison"]
  },
  {
    title: "Data Structures",
    note: "Storing and processing collections.",
    skills: ["tuples", "lists", "indexing", "slicing", "mutation", "aliasing", "cloning", "nested lists"]
  }
];
...
function renderUpdates() {
  ...
  if (!container) return;
  container.innerHTML = updates.map(([date, text]) => `
